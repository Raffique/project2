...
def cars():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            car = Cars(description=description, make=make, model=model, colour=colour, year=year, transmission=transmission, car_type=car_type, price=price, user_id=current_user.get_id(), photo=names)
            db.session.add(car)
            db.session.commit()
            return jsonify({'status': 200})
        else:
    ...
    carfiles = [car.serialize() for car in carfiles]
    return jsonify({'cars': carfiles})
...
def getFavs():
    favs = Favourites.query.filter_by(user_id=current_user.get_id())
    favs = [fav.serialize()['car_id'] for fav in favs]
    favs2  = favs
    lst = []
    for el in favs2:
        if el not in lst:
    ...
    favs = []        
    for el in lst:
        favs.append(Cars.query.filter_by(id=el).first())
    favs = [fav.serialize() for fav in favs]
    return jsonify({'favs': favs})


@app.route("/api/auth/checklogin")
def checklogin():
